    ...
    public partial class frmCotizador : Form
    {
        ...
        private void frmCotizador_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                ConfigurarValidacionesCampos(); // NUEVO: Agregar validaciones
            }
            catch (Exception ex)
            {
        ...
        private void CmbMaterial_SelectedIndexChanged(object sender, EventArgs e, int numeroCombo)
        {
            try
            {
                ...
                else
                {
                    ...
                    LimpiarLabelsMateriasVarios(numeroCombo);
                }
            }
            catch (Exception ex)
            {
        ...
        private void MostrarPrecioMaterialVarios(int numeroCombo, DtoMaterial material)
        {
            ...
            foreach (var nombrePrecio in posiblesNombresPrecio)
            {
                ...
                if (lblPrecio != null)
                {
                    lblPrecio.Text = $"${material.PrecioUnitario:N2} / {material.Unidad ?? "unidad"}";
                    lblPrecio.Visible = true;
                    break;
                }
            }
        ...
        private void RecalcularTotalMaterialVarios(int numeroCombo, DtoMaterial material)
        {
            ...
            foreach (var nombreCantidad in posiblesNombresCantidad)
            {
                ...
                if (txtCantidad != null && int.TryParse(txtCantidad.Text, out int cantidad) && cantidad > 0)
                {
                    ...
                    MostrarTotalMaterialVarios(numeroCombo, total);
                    break;
                }
            }
        }
            ...
            try
            {
                if (sender is TextBox txtCantidad)
                {
                    ...
                    if (cmbMaterial?.SelectedItem is DtoMaterial material)
                    {
                        ...
                        {
                            MostrarTotalMaterialVarios(numeroCombo, 0);
                        }
                    }
                }
            }
        ...
        private void btnCalcularPies_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                // Mostrar total de pies cúbicos
                decimal totalPies = detallesCotizacion.Sum(d => d.PiesCubicos);
                
                // Aplicar desperdicio si está configurado
                ...
                // Mensaje de éxito
                MessageBox.Show($"Cálculo completado. Total: {totalPies:F2} pies cúbicos", "Información", 
                                        ...
                                        Label lblResultado, TextBox txtDescripcion)
        {
            ...
            try
            {
                ...
                // Mostrar resultado
                lblResultado.Text = $"{detalle.PiesCubicos:F2}";
        ...
        private void btnCalcularCotizacion_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                // Crear cotización con todos los costos
        ...
        private decimal CalcularCostoMaterialesVarios()
        {
            ...
            for (int i = 1; i <= 6; i++)
            {
                // Buscar nombres posibles de checkboxes
                ...
                if (chkMaterial?.Checked == true)
                {
                    ...
                    // Buscar TextBox de cantidad con nombres posibles
                    var posiblesNombresCantidad = new[] { 
                        ...
                        int.TryParse(txtCantidad.Text, out int cantidad) && cantidad > 0)
                    {
                        ...
                        // Actualizar label de total del material
                        MostrarTotalMaterialVarios(i, costoTotal);
                    }
                }
            }
        ...
        private decimal CalcularOtrosCostos()
        {
            ...
            // Otros materiales
            var chkOtrosMateriales = this.Controls.Find("chkotrosmateriales", true).FirstOrDefault() as CheckBox;
            var txtOtrosMateriales = this.Controls.Find("txtmateriales", true).FirstOrDefault() as TextBox;
            if (chkOtrosMateriales?.Checked == true && decimal.TryParse(txtOtrosMateriales?.Text, out decimal otrosMat))
            {
                otrosCostos += otrosMat;
                
                var lblOtrosMateriales = this.Controls.Find("lblotrosmateriales", true).FirstOrDefault() as Label;
                if (lblOtrosMateriales != null)
                {
                    lblOtrosMateriales.Text = $"${otrosMat:N2}";
                    lblOtrosMateriales.Visible = true;
                }
            }
            
            // Gastos varios
            var chkGastosVarios = this.Controls.Find("chkgastosvarios", true).FirstOrDefault() as CheckBox;
            var txtGastosVarios = this.Controls.Find("txtgastosvarios", true).FirstOrDefault() as TextBox;
            if (chkGastosVarios?.Checked == true && decimal.TryParse(txtGastosVarios?.Text, out decimal gastosVarios))
            {
                otrosCostos += gastosVarios;
                
                var lblGastosVarios = this.Controls.Find("lbltotalgastosvarios", true).FirstOrDefault() as Label;
                if (lblGastosVarios != null)
                {
                    lblGastosVarios.Text = $"${gastosVarios:N2}";
            ...
            {
                lblTotalGastos.Text = $"${otrosCostos:N2}";
        ...
        private void MostrarDesgloseCostos(decimal costoVidrios, decimal costoMateriales, decimal otrosCostos)
        {
            // Opcional: Mostrar un desglose detallado en algún control o mensaje
            System.Diagnostics.Debug.WriteLine($"Desglose de costos:");
            System.Diagnostics.Debug.WriteLine($"Vidrios: ${costoVidrios:N2}");
            System.Diagnostics.Debug.WriteLine($"Materiales varios: ${costoMateriales:N2}");
            System.Diagnostics.Debug.WriteLine($"Otros costos: ${otrosCostos:N2}");
        }
        #endregion
        ...
        private void LimpiarControlesMaterialesVarios()
        {
            ...
            for (int i = 1; i <= 6; i++)
            {
                ...
                LimpiarLabelsMateriasVarios(i);
            }
        }

        private void LimpiarLabelsMateriasVarios(int numeroCombo)
        {
